...
/**
 * Valida cada fila de un archivo cargado (CSV o JSON) aplicando las 10 Reglas de Negocio obligatorias.
 */
export function validarCargaDatos(
  ...
  rows.forEach((row, idx) => {
    ...
    if (camposFaltantes.length > 0) {
      issues.push({
        issue_id: `DQ-R10-${filaNum}-${Date.now()}`,
        upload_id: 'TEMP',
        fila: filaNum,
    ...
    if (skuId && !validSkuIds.has(skuId)) {
      issues.push({
        issue_id: `DQ-R1-${filaNum}-${Date.now()}`,
        upload_id: 'TEMP',
        fila: filaNum,
        campo: 'sku_id',
        valor: skuId,
        regla: `El SKU (${skuId}) debe existir en el maestro dim_sku.`,
        severidad: 'BLOQUEANTE',
        accion: 'Fila rechazada',
        estado: 'ABIERTO',
      });
    ...
    // REGLA 2: Cantidad debe ser mayor a 0 [ERROR]
    if (rawCantidad !== undefined && rawCantidad !== null && String(rawCantidad).trim() !== '') {
      const numCant = Number(rawCantidad);
      if (isNaN(numCant) || numCant <= 0) {
        issues.push({
          issue_id: `DQ-R2-${filaNum}-${Date.now()}`,
          upload_id: 'TEMP',
          fila: filaNum,
          campo: 'cantidad',
          valor: String(rawCantidad),
          regla: 'La cantidad debe ser un número estrictamente mayor a 0.',
          severidad: 'BLOQUEANTE',
          accion: 'Fila rechazada',
          estado: 'ABIERTO',
        });
    ...
    if (rawLeadTime !== undefined && rawLeadTime !== null && String(rawLeadTime).trim() !== '') {
      ...
      if (isNaN(numLt) || numLt < 0) {
        issues.push({
          issue_id: `DQ-R3-${filaNum}-${Date.now()}`,
          upload_id: 'TEMP',
          fila: filaNum,
          campo: 'lead_time',
    ...
    if (rawFechaOrden && rawFechaRecepcion) {
      ...
      if (!isNaN(dateOrden.getTime()) && !isNaN(dateRecep.getTime())) {
        if (dateRecep < dateOrden) {
          issues.push({
            issue_id: `DQ-R4-${filaNum}-${Date.now()}`,
            upload_id: 'TEMP',
            fila: filaNum,
    ...
    if (skuId && validSkuIds.has(skuId) && rawUnidad) {
      ...
      if (skuMaster && skuMaster.unidad.trim().toLowerCase() !== unidadStr) {
        issues.push({
          issue_id: `DQ-R5-${filaNum}-${Date.now()}`,
          upload_id: 'TEMP',
          fila: filaNum,
          campo: 'unidad',
    ...
    if (periodo && skuId && zonaId) {
      ...
      if (combinacionesSet.has(comboKey)) {
        issues.push({
          issue_id: `DQ-R6-${filaNum}-${Date.now()}`,
          upload_id: 'TEMP',
          fila: filaNum,
    ...
    if (rawProveedorId !== undefined && rawProveedorId !== null && String(rawProveedorId).trim() !== '') {
      ...
      if (!validProveedorIds.has(provStr)) {
        issues.push({
          issue_id: `DQ-R7-${filaNum}-${Date.now()}`,
          upload_id: 'TEMP',
          fila: filaNum,
          campo: 'proveedor_id',
          valor: provStr,
    ...
    if (periodo) {
      ...
      if (!periodoRegex.test(periodo)) {
        issues.push({
          issue_id: `DQ-R8-${filaNum}-${Date.now()}`,
          upload_id: 'TEMP',
          fila: filaNum,
          campo: 'periodo',
          valor: periodo,
    ...
    if (rawEstado !== undefined && rawEstado !== null && String(rawEstado).trim() !== '') {
      ...
      if (!estadosValidos.has(estStr)) {
        issues.push({
          issue_id: `DQ-R9-${filaNum}-${Date.now()}`,
          upload_id: 'TEMP',
          fila: filaNum,
          campo: 'estado',
